...
class FRB(nn.Module):
    ...
    def forward(self, xl, xh):
        ...
        zc_ = zc_.repeat(1, 1, xl.shape[2] * xl.shape[3]).view(
            -1, xl.shape[1], xl.shape[2], xl.shape[3]
        )
        xl_ = zc_ * xl  # n,c,h,w
        return xl_
...
class Video2RollNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            5, 64, kernel_size=(11, 11), stride=(2, 2), padding=(4, 4), bias=False
        )
...
class Video2RollNet_resnet50(nn.Module):
    ...
    def __init__(
        ...
        block=BasicBlock,
        layers=[2, 2, 2, 2],
        top_channel_nums=2048,
        reduced_channel_nums=256,
        num_classes=51,
        scale=1,
    ):
        self.inplanes = 64
...
def resnet18(**kwargs):
    ...
    model = ResNet(
        BasicBlock, layers=[2, 2, 2, 2], top_channel_nums=512, reduced_channel_nums=64, **kwargs
    )
    return model
