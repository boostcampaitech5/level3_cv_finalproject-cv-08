...
class Roll2MidiDataset(Dataset):
    def __init__(
        self, path="../ytdataset", est_roll_path="./outputs_test", train=True, device=DEFAULT_DEVICE
    ):
        self.path = path
    ...
    def load_data(self):
        ...
        test_gt_folders.sort(key=lambda x: int(x.split("/")[-1].split("_")[0]))

        # Roll predictions dir
        train_roll_folder = glob.glob(self.est_roll_path + "training/*")
        train_roll_folder.sort(key=lambda x: int(x.split("/")[-1].split("_")[0]))
        test_roll_folder = glob.glob(self.est_roll_path + "testing/*")
        test_roll_folder.sort(key=lambda x: int(x.split("/")[-1].split("_")[0]))

        # self.folders: dictionary
        # key: train/test, values: list of tuples [(ground truth midi folder name, roll prediction folder name)]
        self.folders = {}
        self.folders["train"] = [
            (train_gt_folders[i], train_roll_folder[i]) for i in range(len(train_gt_folders))
        ]
        self.folders["test"] = [
            (test_gt_folders[i], test_roll_folder[i]) for i in range(len(test_gt_folders))
        ]
        ...
        for train_gt_folder, est_roll_folder in self.folders["train"]:
            ...
            print(
                "have the same files of training gt and est roll:",
                len(gt_files) == len(est_roll_files),
            )
        ...
        for test_gt_folder, est_roll_folder in self.folders["test"]:
            ...
            est_roll_files.sort(key=lambda x: int(x.split("/")[-1].split(".")[0].split("-")[0]))
            print(
                "have the same files of testing midi and roll:",
                len(gt_files) == len(est_roll_files),
            )
            for i in range(len(gt_files)):
                with np.load(gt_files[i]) as data:
