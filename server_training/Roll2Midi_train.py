...
from Roll2MidiNet import Discriminator, Generator, weights_init_normal
...
class hyperparams(object):
    def __init__(self):
        ...
        self.channels = 1
        self.h = 85  # input Piano key ranges
...
def process_data():
    train_dataset = Roll2MidiDataset(
        path="./data/ytdataset/", est_roll_path="./data/v2r_inputs/", train=True
    )
    train_loader = utils.DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=8)
    test_dataset = Roll2MidiDataset(
        path="./data/ytdataset/", est_roll_path="./data/v2r_inputs/", train=False
    )
...
def train(
    ...
    for batch_idx, data in enumerate(train_loader):
        gt, roll = data
        gt, roll = gt.cuda(), roll.cuda()
        # Adversarial ground truths
        valid = Variable(
            Tensor(gt.shape[0], *discriminator.output_shape).fill_(1.0), requires_grad=False
        )
        fake = Variable(
            Tensor(gt.shape[0], *discriminator.output_shape).fill_(0.0), requires_grad=False
        )
        gt = gt.type(Tensor)
        ...
        ) + 0.999 * adversarial_loss(gen_imgs, gt)
        ...
        if batch_idx % 2 == 0:
            print(
                "Train Epoch: {0} [{1}/{2} ({3:.0f}%)]\t g Loss: {4:.6f} | d Loss: {5:.6f}".format(
                    epoch,
                    batch_idx * roll.shape[0],
                    len(train_loader.dataset),
                    100.0 * batch_idx / len(train_loader),
                    g_loss.item() / roll.shape[0],
                    d_loss.item() / roll.shape[0],
                )
            )
    scheduler.step(train_g_loss / len(train_loader.dataset))
    print(
        "====> Epoch: {} Average g loss: {:.4f} | d loss: {:.4f}".format(
            epoch,
...
def test(
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(test_loader):
            ...
            numpy_pre_label_ = np.reshape(numpy_pre_label_, (-1, 85))
...
def main():
    ...
    for epoch in range(hp.train_epoch):
        ...
        g_loss, d_loss = train(
            ...
            scheduler,
            adversarial_loss,
            hp.iter_train_g_loss,
            hp.iter_train_d_loss,
        )
