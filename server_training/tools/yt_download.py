...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="../data/PianoYT/pianoyt.csv",
        help='default="../data/PianoYT/pianoyt.csv"',
    )
    parser.add_argument(
        "--save_path", type=str, default="../data/videos", help='default="../data/videos"'
    )
    ...
    music_list = pd.read_csv(
        args.csv_path,
        names=["index", "link", "train/test", "crop_minY", "crop_maxY", "crop_minX", "crop_maxX"],
    )
    ...
    for i, (is_train, music_link, music_idx) in tqdm(
        music_list[["train/test", "link", "index"]].iterrows(), total=music_list.shape[0]
    ):
        if is_train == 1:
        ...
        try:
            ...
            os.system(
                f'python -m yt_dlp "{music_link}" --paths "{output_path}" -o "{music_idx}_%(title)s"'
            )
