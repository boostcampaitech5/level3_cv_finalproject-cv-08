...
def main(args):
    ...
    for vid_name in vid_list:
        ...
        if args.no_crop:
            vid_obj.filter("fps", fps=25).filter("scale", "900", "100").output(
                os.path.join(vid_save_path, "%04d.png"), **{"qmin": 1, "qmax": 1}
            ).run()
        else:
            minY, maxY, minX, maxX = music_list[music_list["index"] == idx][
                ["crop_minY", "crop_maxY", "crop_minX", "crop_maxX"]
            ].values[0]
            vid_obj.filter("fps", fps=25).filter(
                "crop", maxX - minX, maxY - minY, minX, minY
            ).filter("scale", "900", "100").output(
                os.path.join(vid_save_path, "%04d.png"), **{"qmin": 1, "qmax": 1}
            ).run()
