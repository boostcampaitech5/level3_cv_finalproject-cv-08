...
class Midi_Generation:
    ...
    def inference(self):
        ...
        for i, data in enumerate(self.final_data):
            roll = torch.unsqueeze(
                torch.unsqueeze(torch.sigmoid(data.T.float().cuda()), dim=0), dim=0
            )
            print("piece ", i)
            with torch.no_grad():
                model.eval()
...
if __name__ == "__main__":
    ...
    if args.iter:
        for video_name in os.listdir(est_roll_folder):
            generator = Midi_Generation(
                checkpoints, exp_dir, est_roll_folder, video_name, args.output_path
            )
            generator.inference()
    else:
