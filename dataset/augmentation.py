...
class BaseAugmentation:
    ...
    def __call__(self, image, image1, image2, image3, image4):
        return self.transform(
            image=image, image1=image1, image2=image2, image3=image3, image4=image4
        )
    ...
    def get_transform_list(self):
        return [
            A.RandomBrightnessContrast(
                brightness_limit=[0.9, 0.9], contrast_limit=[-0.1, 0.3], p=self.p
            )
        ]
...
class DummyAugmentation(BaseAugmentation):
    ...
    def get_transform_list(self):
        return [
            A.RandomBrightnessContrast(
                brightness_limit=[1.0, 1.0], contrast_limit=[1.0, 1.0], p=self.p
            )
        ]
...
class Normalize(BaseAugmentation):
    ...
    def get_transform_list(self):
        return [A.Normalize(mean=0.5, std=0.2, p=1.0)]
